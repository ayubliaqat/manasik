...
/**
 * Toolbar button must live outside TiptapEditor.
 * Defining it inside the component creates a new component
 * on every render and causes React's component-during-render error.
 */
...
export function TiptapEditor({
  ...
  // Tiptap editor is initially null.
  // Return before using it anywhere.
  if (!editor) {
    return null
  }

  // Keep a stable non-null reference for TypeScript.
  const tiptapEditor = editor

  function currentHeadingValue() {
    for (let level = 1; level <= 6; level++) {
      if (tiptapEditor.isActive("heading", { level })) {
        return `h${level}`
      }
    }
  ...
  return (
    <div className="rounded-xl border border-soft-beige overflow-hidden">
      ...
      <div className="flex items-center gap-0.5 flex-wrap border-b border-soft-beige bg-soft-beige/30 px-2 py-1.5">
        ...
        <ToolbarButton
          title="Highlight"
          active={tiptapEditor.isActive("highlight")}
          onClick={() =>
        ...
        <ToolbarButton
          title="Quote"
          active={tiptapEditor.isActive("blockquote")}
          onClick={() =>
          ...
          {tableMenuOpen && (
            <div className="absolute z-20 top-9 left-0 w-48 rounded-xl border border-soft-beige bg-card shadow-lg py-1.5">
              <button
                ...
                className="w-full text-left px-3 py-1.5 text-xs text-charcoal hover:bg-soft-beige/60"
              >
                Insert 3x3 table
              </button>
      ...
      {/* Editor */}
      <div
        className="px-4 py-3 bg-warm-white"
